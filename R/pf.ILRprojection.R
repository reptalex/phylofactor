...
pf.ILRprojection <- function(PF,Data=NULL,nfactors=2){
  ...
  if (is.null(Data)){
    output <- lapply(PF$groups[1:nfactors],BalanceContrast,TransformedData=log(PF$Data)) %>% unlist %>% matrix(.,nrow=nfactors,byrow=T)
    ...
    output <- lapply(PF$groups[1:nfactors],BalanceContrast,TransformedData=log(Data)) %>% unlist %>% matrix(.,nrow=nfactors,byrow=T)
